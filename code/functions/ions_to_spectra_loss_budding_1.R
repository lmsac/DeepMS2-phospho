ions.to.spectra.loss.budding.1 = function(
  ions, carbamidomethyl = TRUE, charge = NA, 
  loss.ratio = 0.9, loss.site = c('S', 'T'),
  budding = 5e-3, budding.type = c('b', 'y', 'bp', 'yp'), budding.charge = c(1, 2), 
  ...) {
  ...
  lapply(ions, function(x) {
    ...
    lossFragments = sapply(names(intensity), function(label) {
      if (grepl('^[by]p[0-9]+\\^[0-9]+$', label)) {
        num = as.integer(sub('^[by]p([0-9]+)\\^[0-9]+$', '\\1', label))
        aa = strsplit(x$peptide, '')[[1]]
        if (startsWith(label, 'b') & any(head(aa, num) %in% loss.site)) {
          TRUE
        }
        else if (startsWith(label, 'y') & any(tail(aa, num) %in% loss.site)) {
          TRUE
        }
        else {
          FALSE
    ...
    sapply(names(intensity)[!is.na(lossFragments) & lossFragments], function(label) {
      noloss = sub('^([by])p', '\\1', label)
      if (noloss %in% names(intensity)) {
        intensitySum = intensity[label] + intensity[noloss]
        intensity[label] <<- intensitySum * loss.ratio
        intensity[noloss] <<- intensitySum * (1 - loss.ratio)
      }
    })
    ...
    buddingFragments = grepl(
      ...
      names(mz)
    ) & (is.na(lossFragments) | lossFragments)
