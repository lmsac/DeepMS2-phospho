ions.to.spectra.budding = function(ions, carbamidomethyl = TRUE, charge = NA, budding = 5e-3, budding.type = c('b', 'y', 'bp', 'yp'), budding.charge = c(1, 2), ...) {
  lapply(ions, function(x) {
    ...
    # buddingFragments = grepl('^[by][0-9]+\\^[1]', names(mz))
    # buddingFragments = grepl('^[by][0-9]+\\^[12]', names(mz))
    # buddingFragments = grepl('^[by][p]?[0-9]+\\^[1]', names(mz))
    # buddingFragments = grepl('^[by][p]?[0-9]+\\^[12]', names(mz))
